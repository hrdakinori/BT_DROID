...
#ifndef USB_GENERIC_EP1
    #define USB_GENERIC_EP1       1
#endif
#ifndef USB_GENERIC_EP2
...
#define EVENT_GENERIC_TX2_DONE (EVENT_GENERIC_BASE+EVENT_GENERIC_OFFSET+2)
        ...
        // from the device.
#define EVENT_GENERIC_RX2_DONE (EVENT_GENERIC_BASE+EVENT_GENERIC_OFFSET+3)
        ...
        // to the receive buffer, and size is the actual number of bytes read
        // from the device.
#define EVENT_GENERIC_RX1_DONE (EVENT_GENERIC_BASE+EVENT_GENERIC_OFFSET+4)
...
typedef struct _GENERIC_DEVICE
{
    ...
    DWORD               rx2Length;       // Number of bytes received in the last IN transfer
    DWORD               rx1Length;       // Number of bytes received in the last IN transfer
    BYTE                clientDriverID; // ID to send when issuing a Device Request
    
    #ifndef USB_ENABLE_TRANSFER_EVENT
        BYTE            tx2ErrorCode;    // Error code of last OUT transfer
        BYTE            rx2ErrorCode;    // Error code of last IN transfer
        BYTE            rx1ErrorCode;    // Error code of last IN transfer
    #endif
    ...
    union
    {
        ...
        struct
        {
            ...
            BYTE tx0Busy         : 1;    // Driver busy transmitting data
            BYTE tx2Busy         : 1;    // Driver busy transmitting data
...
#define USBHostGenericGetRx2Length(a) ( (API_VALID(a)) ? gc_DevData.rx2Length : 0 )
//DWORD USBHostGenericGetRx2Length( BYTE deviceAddress );
...
#define USBHostGenericGetRx1Length(a) ( (API_VALID(a)) ? gc_DevData.rx1Length : 0 )
//DWORD USBHostGenericGetRx1Length( BYTE deviceAddress );
...
#define USBHostGenericRead2(a,b,l)                                                 \
        ( API_VALID(deviceAddress) ? USBHostRead((a),USB_GENERIC_EP2,(BYTE *)(b),(l)) : \
                              USB_INVALID_STATE )
*/
/****************************************************************************
  Function:
...
/* Macro Implementation:
#define USBHostGenericRead1(a,b,l)                                                 \
        ( API_VALID(deviceAddress) ? USBHostRead((a),USB_GENERIC_EP1,(BYTE *)(b),(l)) : \
                              USB_INVALID_STATE )
*/
...
#define USBHostGenericTx2IsBusy(a) ( (API_VALID(a)) ? ((gc_DevData.flags.tx2Busy == 1) ? TRUE : FALSE) : TRUE )
//BOOL USBHostGenericTx2IsBusy( BYTE deviceAddress );
...
/****************************************************************************
  ...
  Description:
    ...
    This function is intended for use with polling.  With transfer events,
    the function USBHostGenericTx2IsBusy() should be used.
...
/* Macro Implementation:
#define USBHostGenericWrite2(a,b,l)                                                 \
        ( API_VALID(deviceAddress) ? USBHostWrite((a),USB_GENERIC_EP2,(BYTE *)(b),(l)) : \
                              USB_INVALID_STATE )
*/
