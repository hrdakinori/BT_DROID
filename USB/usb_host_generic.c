...
BOOL USBHostGenericInit ( BYTE address, DWORD flags, BYTE clientDriverID )
{
    ...
    // Initialize state
    gc_DevData.rx2Length     = 0;
    gc_DevData.rx1Length     = 0;
...
BOOL USBHostGenericEventHandler ( BYTE address, USB_EVENT event, void *data, DWORD size )
{
    ...
    case EVENT_TRANSFER:
        if ( (data != NULL) && (size == sizeof(HOST_TRANSFER_DATA)) )
        {
            ...
            if ( ((HOST_TRANSFER_DATA *)data)->bEndpointAddress == (USB_IN_EP|USB_GENERIC_EP2) )
            {
                gc_DevData.flags.rx2Busy = 0;
                gc_DevData.rx2Length = dataCount;
                USB_HOST_APP_EVENT_HANDLER(gc_DevData.ID.deviceAddress, EVENT_GENERIC_RX2_DONE, &dataCount, sizeof(DWORD) );
            }
            else if ( ((HOST_TRANSFER_DATA *)data)->bEndpointAddress == (USB_OUT_EP|USB_GENERIC_EP2) )
            {
                gc_DevData.flags.tx2Busy = 0;
                USB_HOST_APP_EVENT_HANDLER(gc_DevData.ID.deviceAddress, EVENT_GENERIC_TX2_DONE, &dataCount, sizeof(DWORD) );
            }
            else if ( ((HOST_TRANSFER_DATA *)data)->bEndpointAddress == (USB_IN_EP|USB_GENERIC_EP1) )
            {
                gc_DevData.flags.rx1Busy = 0;
                gc_DevData.rx1Length = dataCount;
...
/****************************************************************************
  Function:
    DWORD USBHostGenericGetRx2Length( BYTE deviceAddress )
...
/****************************************************************************
  Function:
    DWORD USBHostGenericGetRx1Length( BYTE deviceAddress )
...
/****************************************************************************
  Function:
    void USBHostGenericRead2( BYTE deviceAddress, BYTE *buffer, DWORD length )
  ...
  Example:
    ...
    {
        USBHostGenericRead2( deviceAddress, &buffer, sizeof(buffer) );
    }
    </code>
...
BYTE USBHostGenericRead2( BYTE deviceAddress, void *buffer, DWORD length )
{
    BYTE RetVal;
...
/****************************************************************************
  Function:
    void USBHostGenericRead1( BYTE deviceAddress, BYTE *buffer, DWORD length )
  ...
  Example:
    ...
    {
        USBHostGenericRead1( deviceAddress, &buffer, sizeof(buffer) );
    }
    </code>
...
BYTE USBHostGenericRead1( BYTE deviceAddress, void *buffer, DWORD length )
{
    BYTE RetVal;
    ...
    gc_DevData.rx1Length = 0;
    RetVal = USBHostRead( deviceAddress, USB_IN_EP|USB_GENERIC_EP1, (BYTE *)buffer, length );
    if (RetVal != USB_SUCCESS)
    {
...
/****************************************************************************
  Function:
    BOOL USBHostGenericRx2IsBusy( BYTE deviceAddress )
  ...
  Description:
    ...
    transfer events.  With polling, the function USBHostGenericRx2IsComplete()
    should be used.
...
/****************************************************************************
  Function:
    BOOL USBHostGenericRx1IsBusy( BYTE deviceAddress )
  ...
  Description:
    This interface is used to check if the client driver is currently busy
    receiving data from the device.  This function is intended for use with
    transfer events.  With polling, the function USBHostGenericRx1IsComplete()
    should be used.
...
/****************************************************************************
  Function:
    BOOL USBHostGenericRx2IsComplete( BYTE deviceAddress, BYTE *errorCode,
                DWORD *byteCount )
  ...
  Description:
    ...
    the function USBHostGenericRx2IsBusy() should be used.
...
BOOL USBHostGenericRx2IsComplete( BYTE deviceAddress,
                                    BYTE *errorCode, DWORD *byteCount )
{
    if (gc_DevData.flags.rx2Busy)
    {
        return FALSE;
    }
    else
    {
        *byteCount = gc_DevData.rx2Length;
        *errorCode = gc_DevData.rx2ErrorCode;
        return TRUE;
    }
}
#endif
...
/****************************************************************************
  Function:
    BOOL USBHostGenericRx1IsComplete( BYTE deviceAddress, BYTE *errorCode,
                DWORD *byteCount )
...
#ifndef USB_ENABLE_TRANSFER_EVENT
BOOL USBHostGenericRx1IsComplete( BYTE deviceAddress,
                                    BYTE *errorCode, DWORD *byteCount )
{
    if (gc_DevData.flags.rx1Busy)
    {
        return FALSE;
    }
    else
    {
        *byteCount = gc_DevData.rx1Length;
        *errorCode = gc_DevData.rx1ErrorCode;
        return TRUE;
    }
}
#endif
...
void USBHostGenericTasks( void )
{
    ...
    if (gc_DevData.ID.deviceAddress && gc_DevData.flags.initialized)
    {
        ...
        if (gc_DevData.flags.rx1Busy)
        {
            if (USBHostTransferIsComplete( gc_DevData.ID.deviceAddress, USB_IN_EP|USB_GENERIC_EP1, &errorCode, &byteCount ))
            {
                gc_DevData.flags.rx1Busy = 0;
                gc_DevData.rx1Length     = byteCount;
                gc_DevData.rx1ErrorCode  = errorCode;
            }
        }

        if (gc_DevData.flags.rx2Busy)
        {
            if (USBHostTransferIsComplete( gc_DevData.ID.deviceAddress, USB_IN_EP|USB_GENERIC_EP2, &errorCode, &byteCount ))
            {
                gc_DevData.flags.rx2Busy = 0;
                gc_DevData.rx2Length     = byteCount;
                gc_DevData.rx2ErrorCode  = errorCode;
            }
        }

        if (gc_DevData.flags.tx2Busy)
        {
            if (USBHostTransferIsComplete( gc_DevData.ID.deviceAddress, USB_OUT_EP|USB_GENERIC_EP2, &errorCode, &byteCount ))
            {
                gc_DevData.flags.tx2Busy = 0;
...
/****************************************************************************
  Function:
    BOOL USBHostGenericTx2IsBusy( BYTE deviceAddress )
  ...
  Example:
    <code>
    if (!USBHostGenericTx2IsBusy( deviceAddress ) )
    {
        USBHostGenericWrite2( deviceAddress, &buffer, sizeof( buffer ) );
    }
    </code>
...
/****************************************************************************
  Function:
    BOOL USBHostGenericTx2IsComplete( BYTE deviceAddress, BYTE *errorCode )
...
/****************************************************************************
  Function:
    void USBHostGenericWrite2( BYTE deviceAddress, BYTE *buffer, DWORD length )
...
BYTE USBHostGenericWrite2( BYTE deviceAddress, void *buffer, DWORD length )
{
    BYTE RetVal;
	BYTE errorCode;
	DWORD byteCount;
    ...
    // Validate the call
    if (!API_VALID(deviceAddress)) return USB_INVALID_STATE;
    if (gc_DevData.flags.tx2Busy)   return USB_BUSY;

		#ifdef DEBUG_MODE
		int data_num;
		UART2PrintString( "Write2: " );	
		for(data_num=0;data_num<length;data_num++)
        {UART2PutHex(*((BYTE*)buffer+data_num));UART2PutChar(' ');}
		UART2PrintString( "\r\n" );
		#endif

    // Set the busy flag and start a new OUT transfer.
    gc_DevData.flags.tx2Busy = 1;
	...
	do
	{
		ret = USBHostTransferIsComplete(deviceAddress, USB_OUT_EP|USB_GENERIC_EP2,&errorCode,&byteCount);
	}while(ret == FALSE);
    gc_DevData.flags.tx2Busy = 0;
...
/****************************************************************************
  Function:
    void USBHostGenericWrite0( BYTE deviceAddress, void *buffer, WORD wLength )
  ...
  Parameters:
    deviceAddress   - USB Address of the device.
    buffer          - Pointer to the data buffer
    wlength         - Number of bytes to be transferred
  ...
  Remarks:
    None
  ***************************************************************************/
BYTE USBHostGenericWrite0( BYTE deviceAddress, void *buffer, WORD wLength )
{
    BYTE RetVal;

    // Validate the call
    if (!API_VALID(deviceAddress)) return USB_INVALID_STATE;
//    if (gc_DevData.flags.tx0Busy)   return USB_BUSY;

		#ifdef DEBUG_MODE
		int data_num;
		UART2PrintString( "Write0: " );	
		for(data_num=0;data_num<wLength;data_num++)
        {UART2PutHex(*((BYTE*)buffer+data_num));UART2PutChar(' ');}
		UART2PrintString( "\r\n" );
		#endif

    // Set the busy flag and start a new OUT transfer.
//    gc_DevData.flags.tx0Busy = 1;
//    RetVal = USBHostWrite( deviceAddress, USB_OUT_EP|0, (BYTE *)buffer, wLength );
    RetVal = USBHostIssueDeviceRequest( deviceAddress, 0x21,0, 0, 0, wLength, buffer, USB_DEVICE_REQUEST_SET,0x00);
    if (RetVal != USB_SUCCESS)
    {
//        gc_DevData.flags.tx0Busy = 0;    // Clear flag to allow re-try
    }
